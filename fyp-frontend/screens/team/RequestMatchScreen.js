...
const RequestMatchScreen = ({ navigation, route }) => {
  ...
  const { teams, getUserTeams, createMatchRequest } = useTeam()
  const { createMatchOpening } = useSolo()
  ...
  useEffect(() => {
    ...
    // Filter out the user's team from available teams
  ...
  const handleSendRequest = () => {
    if (!selectedTeam || !location) return
    ...
    // Create match request using context
    const targetTeam = teams.find((team) => team.id === selectedTeam)
    const requestData = {
      requestingTeamId: teamId,
      targetTeamId: selectedTeam,
      targetTeamName: targetTeam.name,
      targetTeamLogo: targetTeam.logo,
      date: matchDateTime,
      location,
      message,
      format,
    ...
    // Create match openings for any unselected members
    const unselectedMembers = teamMembers.filter((member) => !selectedMembers.includes(member.id))
    if (unselectedMembers.length > 0) {
      // Create positions for unselected members
      const positions = unselectedMembers.map((member) => ({
        id: `pos-${Date.now()}-${member.id}`,
        role: member.role === "Captain" ? "Defender" : member.role, // Default to Defender for captain
        required: true,
      }))

      // Create match opening
      createMatchOpening(
        {
          teamId,
          ...
          opponent: targetTeam.name,
          date: matchDateTime,
          location,
          format,
          captain: teamMembers.find((member) => member.role === "Captain"),
        },
        positions,
      )
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        ...
        <TouchableOpacity
          style={[styles.sendButton, (!selectedTeam || !location) && styles.disabledButton]}
          onPress={handleSendRequest}
          disabled={!selectedTeam || !location || isLoading}
        >
